...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest* request) {
    request->send(SD, "/interfaz/Persiana.html");
  });
  server.on("/logo1", HTTP_GET, [](AsyncWebServerRequest* request) {
    request->send(SD, "/interfaz/logo1.png","image/png");
  });
  server.on("/logo2", HTTP_GET, [](AsyncWebServerRequest* request) {
    request->send(SD, "/interfaz/logo2.png","image/png");
  });
  server.on("/logo", HTTP_GET, [](AsyncWebServerRequest* request) {
    request->send(SD, "/interfaz/logo.png","image/png");
  });
  server.begin();
  webSocket.begin();
